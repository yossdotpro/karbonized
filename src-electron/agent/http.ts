...
import { redactSecrets } from '../../src/lib/agent/core/redact';
import type { HttpBridgeEvent } from '../../src/lib/agent/bridge';
import type { HttpRequest } from '../../src/lib/agent/core/types';
import { getKeyFor, hasKey, removeKey, setKey } from './keys';

/**
 * Model provider requests for Agent, made from the main process: no CORS
...
export const registerAgentHttp = () => {
	const controllers = new Map<string, AbortController>();

	ipcMain.handle('agent:keys:set', (_event, profileId, key, baseUrl) =>
		setKey(profileId, key, baseUrl),
	);
	ipcMain.handle('agent:keys:remove', (_event, profileId) =>
		removeKey(profileId),
	);
	ipcMain.handle('agent:keys:has', (_event, profileId, baseUrl) =>
	...
	ipcMain.on('agent:http:abort', (_event, requestId: unknown) => {
