...
import { getActiveProfile, getTransport, useAgentSettings } from './settings';
import { editorTools } from './tools';
import { useAgentUI } from './ui-store';
import {
	type StreamDraft,
	type ToolMeta,
	emptyDraft,
...
/**
 * Agent conversations: the messages sent to the model, the run in progress
...
const storage = localforage.createInstance({
	name: 'karbonized',
	storeName: 'beedly-conversations', // legacy name, keeps saved chats,
});
...
export const useAgentConversation = create<ConversationState>((set, get) => {
	...
	const persist = async (conversation: Conversation) => {
		...
		try {
			...
			console.error('Could not save the agent conversation:', error);
		}
	};

	const run = async () => {
		const profile = getActiveProfile(useAgentSettings.getState());
		...
		useAgentUI.setState({ working: true });
		set({
			running: true,
		...
		try {
				...
				switch (event.type) {
					...
					case 'finish':
						if (event.reason === 'max-steps') {
							apply({
								error:
									'The agent stopped after too many steps. Send a message to continue.',
							});
						}
						break;
				}
			...
			controller = null;
			useAgentUI.setState({ working: false });
			set({
