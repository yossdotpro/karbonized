...
import type { FilesBridge } from '@/lib/persistence/desktop-files';

/**
 * API the Electron preload exposes as `window.karbonized.agent`. Absent on
...
declare global {
	interface Window {
		karbonized?: { agent?: AgentBridge; files?: FilesBridge };
	}
}

export const getAgentBridge = (): AgentBridge | undefined =>
	typeof window === 'undefined' ? undefined : window.karbonized?.agent;
