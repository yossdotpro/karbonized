...
import type { BeedlyBridge, HttpBridgeEvent } from './bridge';
...
describe('settings store', () => {
	...
	it('creates profiles from presets and activates the first one', () => {
		const store = useBeedlySettings.getState();
		const first = store.addProfile('anthropic');
		const second = useBeedlySettings.getState().addProfile('anthropic');
		...
		expect(useBeedlySettings.getState().activeProfileId).toBe(first.id);
		expect(createProfile('ollama').supportsImages).toBe(false);
	});

	it('tracks keys and switches the active profile when removed', async () => {
		const a = useBeedlySettings.getState().addProfile('openai');
		const b = useBeedlySettings.getState().addProfile('ollama');

		await useBeedlySettings.getState().saveKey(a.id, '  sk-test  ');
		expect(useBeedlySettings.getState().profilesWithKey).toEqual([a.id]);

		await useBeedlySettings.getState().refreshKeys();
		expect(useBeedlySettings.getState().profilesWithKey).toEqual([a.id]);

		await useBeedlySettings.getState().removeProfile(a.id);
		const state = useBeedlySettings.getState();
