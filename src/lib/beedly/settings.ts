...
/**
 * Beedly settings: provider profiles (without keys) and the active one.
 */

interface BeedlySettingsState {
...
export type BeedlySettings = BeedlySettingsState & BeedlySettingsActions;
...
export const useBeedlySettings = create<BeedlySettings>()(
	persist(
		(set, get) => ({
			profiles: [],
		...
		{
			name: 'karbonized:beedly-settings',
			version: 1,
...
export const getActiveProfile = (
	state: Pick<BeedlySettingsState, 'profiles' | 'activeProfileId'>,
...
export const profileProblem = (
	...
	if (profile.model.trim() === '') return 'Choose a model in Beedly settings.';
	if (getPreset(profile.kind).requiresKey && !hasKey) {
		return `Add your ${getPreset(profile.kind).label} API key in Beedly settings.`;
	}
	return null;
};
