...
export const Transcript: React.FC = () => {
	const current = useBeedlyConversation((state) => state.current);
	const running = useBeedlyConversation(
		(state) =>
			state.running && state.runningConversationId === state.current.id,
	);
	const draft = useBeedlyConversation((state) => state.draft);
	const runningToolId = useBeedlyConversation((state) => state.runningToolId);
	const retry = useBeedlyConversation((state) => state.retry);
	const anyRunning = useBeedlyConversation((state) => state.running);
	const lastRun = useBeedlyConversation((state) => state.lastRun);
