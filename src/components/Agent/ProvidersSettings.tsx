...
import { PROVIDER_PRESETS, getPreset } from '@/lib/agent/providers/presets';
import { type ProviderProfile, listModels } from '@/lib/agent/core/client';
import type { ModelInfo } from '@/lib/agent/core/types';
import { getKeyStore } from '@/lib/agent/keys';
import { getTransport, useAgentSettings } from '@/lib/agent/settings';
...
const ApiKeyField: React.FC<{ profile: ProviderProfile }> = ({ profile }) => {
	...
	const hasKey = useAgentSettings((state) =>
		state.profilesWithKey.includes(profile.id),
	);
	const saveKey = useAgentSettings((state) => state.saveKey);
	const removeKey = useAgentSettings((state) => state.removeKey);
...
const ModelField: React.FC<{ profile: ProviderProfile }> = ({ profile }) => {
	const updateProfile = useAgentSettings((state) => state.updateProfile);
...
const ProfileForm: React.FC<{ profile: ProviderProfile }> = ({ profile }) => {
	const preset = getPreset(profile.kind);
	const activeProfileId = useAgentSettings((state) => state.activeProfileId);
	const updateProfile = useAgentSettings((state) => state.updateProfile);
	const removeProfile = useAgentSettings((state) => state.removeProfile);
		...
		<div className='flex flex-col gap-4'>
			...
			<label className='flex items-center justify-between gap-3'>
				<span className='flex flex-col'>
					...
					<span className='text-[11px] text-muted-foreground'>
						Lets Agent look at the canvas to check its work.
					</span>
				</span>
				<Switch
			...
			<div className='flex items-center gap-2 border-t border-border pt-4'>
				...
				<div className='ml-auto flex gap-2'>
					...
					>
						{activeProfileId === profile.id ? 'In use' : 'Use for the agent'}
					</Button>
				</div>
			</div>
		</div>
...
export const ProvidersSettings: React.FC = () => {
	...
	const activeProfileId = useAgentSettings((state) => state.activeProfileId);
	const addProfile = useAgentSettings((state) => state.addProfile);
